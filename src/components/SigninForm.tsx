...
import { User, Lock, Mail, ArrowRight, Loader2 } from "lucide-react"; // Importing icons for visual appeal
...
export default function SigninForm() {
  ...
  // Color Palette & Design Choices (Pure Tailwind):
  // Accent: amber-800
  // Background: stone-50, yellow-50 (for a warm gradient)
  // Text: gray-900, stone-600
  ...
  return (
    ...
    <div className="min-h-screen relative overflow-hidden flex items-center justify-center p-4 bg-stone-50">
      ...
      {/* Abstract Shapes (Soft, Organic Blobs) */}
      <div className="absolute top-1/4 left-0 w-80 h-80 bg-amber-400 opacity-10 rounded-full mix-blend-multiply filter blur-3xl"></div>
      <div className="absolute bottom-1/4 right-0 w-96 h-96 bg-stone-500 opacity-10 rounded-full mix-blend-multiply filter blur-3xl"></div>
      ...
      <div className="relative z-10 w-full max-w-md bg-white p-10 rounded-2xl shadow-2xl border border-stone-100 backdrop-blur-sm">
        ...
        <div className="text-center mb-8">
          <User className="w-10 h-10 text-amber-800 mx-auto mb-3" />
          <h1 className="text-3xl font-extrabold font-serif text-gray-900 mb-1">
            Welcome
          </h1>
          <p className="text-stone-600 text-lg">
            Sign in to manage your premium account.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Sign In Button (Themed like the "Apply for EMI" button in the reference image) */}
          <button
            type="submit"
            disabled={loading}
        ...
        {/* Footer Link (Themed like the "Contact Us" button/link in the reference image) */}
