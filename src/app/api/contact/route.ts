  ...
  try {
    ...
    if (error instanceof Error && error.message.includes("E11000")) {
      return NextResponse.json(
        {
          error:
            "It seems you have already submitted this message. Please wait while we process your request.",
        },
        { status: 400 }
      );
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const adminMailOptions = {
      ...
      subject: `New Contact Form Submission from ${updatedMessage.name}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #A97C51;">New Contact Form Submission</h2>
          <div style="background: #f9f9f9; padding: 20px; border-radius: 10px; border-left: 4px solid #A97C51;">
            <p><strong>Name:</strong> ${updatedMessage.name}</p>
            <p><strong>Email:</strong> ${updatedMessage.email}</p>
            <p><strong>Phone Number:</strong> ${updatedMessage.number || "Not provided"}</p>
            <p><strong>Address:</strong> ${updatedMessage.address || "Not provided"}</p>
            <p><strong>Description:</strong></p>
            <p style="background: white; padding: 15px; border-radius: 5px; border: 1px solid #ddd;">${updatedMessage.description}</p>
            <p style="margin-top: 10px; font-size: 12px; color: #666;">
              <strong>Submission ID:</strong> ${updatedMessage._id}<br>
              <strong>Received:</strong> ${new Date().toLocaleString()}
            </p>
          </div>
          <p style="margin-top: 20px; color: #666; font-size: 14px;">
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background: #f9f9f9; padding: 20px; border-radius: 10px; border-left: 4px solid #A97C51;">
            ...
            <p style="margin-top: 15px; font-size: 12px; color: #666;">
              <strong>Reference ID:</strong> ${updatedMessage._id}<br>
              <strong>Submitted:</strong> ${new Date().toLocaleString()}
            </p>
          </div>
